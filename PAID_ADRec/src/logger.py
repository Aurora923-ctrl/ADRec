...
def reset_log(log_path):
    import logging
    # 使用UTF-8编码打开日志文件，避免Windows系统GBK编码问题
    fileh = logging.FileHandler(log_path, 'a', encoding='utf-8')
...
def cmdline_args():
    ...
    parser.add_argument('--diffusion_loss_type', type=str, choices=['mse', 'cosine'], 
                        help='Diffusion reconstruction loss type: mse (original) or cosine (Phase 1 improvement)')
    parser.add_argument('--pref_loss_scale', type=float, help='Scaling factor for the PreferDiff-style ranking loss (Phase 2 improvement)')
    
    # RoDif arguments
    parser.add_argument('--use_rope', type=str2bool, help='Whether to use RoPE in Transformer')
    parser.add_argument('--position_aware_noise', type=str2bool, help='Whether to use position-aware noise schedule')
    parser.add_argument('--noise_beta', type=float, help='Decay factor for position-aware noise')
...
def load_config(config_file):
    # 模拟加载配置
    # 使用UTF-8编码打开文件，避免Windows系统GBK编码问题
