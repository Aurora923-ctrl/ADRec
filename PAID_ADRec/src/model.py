...
class Att_Diffuse_model(nn.Module):
    ...
    def preferdiff_loss(self, out_seq, labels, neg_samples=1):
        """
        Calculates a BPR-style ranking loss based on the diffusion model's output (Phase 2 improvement).
        L_pref = -log σ(cos(z, e_pos) - cos(z, e_neg))
        """
        # 1. Identify valid positions (where labels > 0)
        mask = (labels > 0).view(-1)

        # Flatten sequences and labels to work with individual items
        # out_seq: [B, L, H] -> [B*L, H]
        # labels: [B, L] -> [B*L]
        z = out_seq.view(-1, self.emb_dim)
        pos_labels = labels.view(-1)

        # Filter only the valid items
        z = z[mask == 1]
        pos_labels = pos_labels[mask == 1]

        if z.size(0) == 0:
            # No positive items in this batch
            return torch.tensor(0.0, device=out_seq.device)

        # 2. Get positive item embeddings
        # e_pos: [N, H], where N is the number of positive items
        e_pos = self.item_embedding(pos_labels)

        # 3. Sample negative items
        # For each positive item, sample `neg_samples` negative items
        neg_labels = torch.randint(
            1, self.item_num + 1,
            (z.size(0), neg_samples),
            device=out_seq.device
        )
        # e_neg: [N, S, H], where S is neg_samples
        e_neg = self.item_embedding(neg_labels)

        # Normalize vectors for cosine similarity calculation
        z_norm = F.normalize(z, p=2, dim=-1)
        e_pos_norm = F.normalize(e_pos, p=2, dim=-1)
        e_neg_norm = F.normalize(e_neg, p=2, dim=-1)

        # 4. Calculate scores
        # cos(z, e_pos): [N]
        score_pos = (z_norm * e_pos_norm).sum(dim=-1)
        # cos(z, e_neg): [N, S]
        # Unsqueeze z_norm to [N, 1, H] for broadcasting with e_neg_norm
        score_neg = (z_norm.unsqueeze(1) * e_neg_norm).sum(dim=-1)

        # 5. Calculate BPR loss
        # For each positive, we might have multiple negatives. Average the loss over negatives.
        # -log σ(pos - neg)
        loss = -F.logsigmoid(score_pos.unsqueeze(1) - score_neg).mean()
